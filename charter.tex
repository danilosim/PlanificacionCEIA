...
\begin{itemize}
  ...
  \item 2–3 puntos: tarea con algo de procesamiento o exploración de datos.
...
\begin{itemize}
  \item \textbf{Épica 1: Relevamiento y análisis de datos históricos}
    \begin{itemize}
      \item \textbf{HU1:} Como analista de datos, quiero explorar y limpiar el dataset histórico para asegurar que los datos sean utilizables para modelado.  
  ...
  \item \textbf{Épica 3: Entrenamiento y validación de modelos}
    \begin{itemize}
      \item \textbf{HU5:} Como desarrollador de modelos, quiero entrenar un modelo de clasificación para predecir si el préstamo es suficiente, y evaluar su rendimiento con métricas como F1-score y AUC.  
      \newline \textbf{Prioridad: Alta} — \textbf{Story Points: 5}

      \item \textbf{HU6:} Como desarrollador de modelos, quiero entrenar un modelo de regresión para estimar los costos por partida presupuestaria, y analizar su precisión mediante MAE y RMSE.  
      \newline \textbf{Prioridad: Alta} — \textbf{Story Points: 5}
    \end{itemize}

  \item \textbf{Épica 4: Interpretación, documentación y validación}
    \begin{itemize}
      \item \textbf{HU7:} Como analista, quiero aplicar técnicas de interpretabilidad (SHAP, Feature Importance) para entender las variables que más influyen en las predicciones.  
...
\begin{itemize}
  \item \textbf{Épica 1: Relevamiento y análisis de datos históricos}
    \begin{itemize}
      ...
      \begin{itemize}
        \item Se ha identificado al menos un conjunto de 10 variables relevantes para predicción.
  ...
  \item \textbf{Épica 2: Preprocesamiento y preparación de datos}
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item Se aplicó una técnica de balanceo (undersampling, oversampling o SMOTE).
        \item Se verificó que el modelo no pierda precisión tras aplicar balanceo.
      \end{itemize}
      \item \textbf{Criterios de aceptación HU4}
      \begin{itemize}
        \item Todas las variables categóricas han sido codificadas mediante one-hot encoding o similar.
  ...
  \item \textbf{Épica 3: Entrenamiento y validación de modelos}
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item Se reportan métricas F1-score y AUC en el conjunto de prueba.
        \item Se alcanza un F1-score mayor al 0.70 o el valor base definido como mínimo aceptable.
      \end{itemize}
      \item \textbf{Criterios de aceptación HU6}
      \begin{itemize}
        \item Se ha entrenado un modelo de regresión con variables de entrada preprocesadas.
        \item Se calculan métricas MAE, RMSE y R² sobre el conjunto de prueba.
        \item El modelo logra un R² superior a 0.80 y un MAE dentro del umbral definido según el negocio.
      \end{itemize}
    \end{itemize}
  ...
  \item \textbf{Épica 4: Interpretación, documentación y validación}
    \begin{itemize}
      ...
      \begin{itemize}
        \item Se aplicó una técnica de explicabilidad (ej. SHAP) sobre los modelos entrenados.
        \item Se identificaron las 5 variables más influyentes en las predicciones.
...
\begin{enumerate}
  ...
  \textbf{Métricas de éxito:} precisión del modelo (F1-score y AUC para clasificación, MAE y R² para regresión), interpretabilidad y aplicabilidad práctica para el analista.
  ...
  \begin{itemize}
    ...
    \item \textbf{Cantidad:} cientos a miles de registros, cada uno con más de 30 variables.
    \item \textbf{Calidad:} en general buena, pero con presencia de valores faltantes, codificación inconsistente de categorías y posibles outliers.
  \end{itemize}

  \item \textbf{Preparación de los datos:}  
  Esta etapa incluye limpieza y transformación del dataset para su uso por los modelos:
  \begin{itemize}
    ...
    \item Codificación de variables categóricas (one-hot encoding).
    \item Normalización y estandarización de variables numéricas.
    \item Balanceo de clases para la variable objetivo en el modelo de clasificación.
    \item Selección de variables relevantes mediante análisis exploratorio y técnicas automáticas (feature importance, selección recursiva).
  \end{itemize}

  \item \textbf{Modelado:}  
  Se abordan dos problemas distintos:
  \begin{itemize}
    \item \textbf{Clasificación binaria:} predecir si el préstamo será suficiente.
    \item \textbf{Regresión multivariada:} estimar el costo por partida presupuestaria.
  \end{itemize}
  Los algoritmos candidatos incluyen:
  \begin{itemize}
    \item \textbf{Para clasificación:} Random Forest, XGBoost, Regresión Logística.
    \item \textbf{Para regresión:} XGBoost Regressor, LightGBM, redes neuronales densas.
  \end{itemize}
  ...
  \begin{itemize}
    \item \textbf{Clasificación:} F1-score, precisión, recall, AUC-ROC.
    \item \textbf{Regresión:} MAE (error absoluto medio), RMSE (raíz del error cuadrático medio), R².
  \end{itemize}
  ...
  Dado que el proyecto es académico, no se contempla un despliegue productivo.  
  No obstante, se documentará cómo podría integrarse el modelo en un pipeline de análisis crediticio dentro de Built Technologies, incluyendo:
  \begin{itemize}
...
La estimación en horas se basa en una evaluación del grado de dificultad técnica, la complejidad algorítmica involucrada, la posible necesidad de investigación exploratoria, y el nivel de incertidumbre asociado a cada actividad. En todos los casos se evitó la subestimación, manteniendo un rango de entre 2 y 8 horas por tarea. Las tareas que excedían dicho umbral fueron fraccionadas para asegurar mayor control en la gestión del tiempo.
...
% TAREAS OPCIONALES / COMPLEMENTARIAS
